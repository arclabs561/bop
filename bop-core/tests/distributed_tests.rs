...
use bop_agent_core::session::{Session, SessionStore};
use bop_agent_core::storage::{ClusterRegistry, KnowledgeItem, KnowledgeStore};
...
async fn test_redb_latency_invariants() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Read latency too high for redb"
    );
...
async fn test_session_persistence_redb() -> anyhow::Result<()> {
    ...
    session.add_message(bop_agent_core::llm::Message::user(
