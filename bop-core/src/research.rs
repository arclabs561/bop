...
impl ResearchAgent {
    ...
    pub async fn research(&mut self, topic: &str) -> Result<String> {
        if self.inner().config().use_bop {
            return self.research_bop(topic).await;
        }
        let query = format!(
    ...
    /// Internal research using Bop deep research pattern
    async fn research_bop(&mut self, topic: &str) -> Result<String> {
        // This is where we'd implement the multi-agent planning/critique loop
        // similar to bop/examples/deep_research.rs
