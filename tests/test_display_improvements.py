...
from bop.display_helpers import format_trust_summary, format_source_credibility, format_clique_clusters, create_trust_table
...
async def test_belief_extraction():
    ...
    response1 = await agent.chat("I think trust is important for knowledge systems")
    
    assert len(agent.prior_beliefs) > 0
    assert any("trust" in b.get("text", "").lower() for b in agent.prior_beliefs)
    
    # Another message
    response2 = await agent.chat("I believe uncertainty affects decision making")
