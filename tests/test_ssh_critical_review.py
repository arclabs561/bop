...
from bop.information_bottleneck import filter_with_information_bottleneck, compute_mutual_information_estimate
...
async def test_resource_triple_invariants_strict():
    ...
    if rt["depth"] > 0:
        assert rt["width"] > 0, f"depth > 0 but width = 0"
...
def test_dataset_expectations_validation():
    ...
    expected_early_stop = simple_query.get("expected_early_stop", True)
...
async def test_ib_filtering_integration_actually_called():
    ...
    with patch('bop.llm.filter_with_information_bottleneck') as mock_filter:
        ...
        result = await llm_service.synthesize_tool_results(
...
def test_dataset_expected_vs_actual_validation():
    ...
    for item in dataset[:3]:
        ...
        assert isinstance(expected_early_stop, bool), f"expected_early_stop should be boolean"
