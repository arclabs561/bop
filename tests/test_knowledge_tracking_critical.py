...
from bop.knowledge_tracking import KnowledgeTracker, ConceptLearning, SessionKnowledge
...
def test_session_id_changes_mid_tracking():
    ...
    session_evolution = tracker.get_session_evolution("session1")
...
def test_cross_session_evolution_with_deleted_sessions():
    ...
    trust_evolution = next(e for e in evolution if e["concept"] == "trust")
...
def test_concept_learning_response_count_not_tracked():
    ...
    learning = tracker.concept_learning.get("trust")  # If trust was extracted
...
async def test_agent_tracking_with_empty_response():
    ...
    response = await agent.chat("What is X?", use_research=False)
